...
dat_old <- googlesheets4::read_sheet(ss=url, na = "NaN", col_types = "cncccnccnnn") %>%
  mutate(species_id = NA) %>%
  mutate(citation_id = NA)
1

dat <- read.csv("/fs/data3/ecowdery/ED.Hydro/parameters/constraint_calculations/constraint_data/conductivity_BETY.csv", na.strings = NaN,  stringsAsFactors = FALSE)
dat$NEW <- 1
dat <- dat %>% rename(species = Species, wood_Kmax = ksat, wood_psi50 = P50, wood_density = WD)
unique(dat$Ref)

dat_old$species <- tolower(dat_old$species)
dat_old$OLD <- 1

dat_test <- full_join(dat, dat_old, by = c("species","wood_Kmax","wood_psi50"))
sum(dat_test$OLD, na.rm = TRUE)
# dat_test %>% filter(citation_author == "Gartner" | Ref == "Gartner et al. 1990")

dat_test %>% filter(OLD == 1 & is.na(NEW)) %>% select(one_of("citation_author", "citation_title", "citation_year")) %>% unique
dat_test %>% filter(OLD == 1 & is.na(NEW))

dat_test %>% filter(NEW == 1 & is.na(OLD)) %>% pull(Ref) %>% unique()
sum(dat_test$OLD, na.rm = TRUE)
...
dat <- inner_join(dat_test, sp, by = "species")

sum(dat$OLD, na.rm = TRUE)


# dat2 <- inner_join(dat_old, sp, by = "species")
...
PFT_species <- tbl(bety, "pfts") %>% dplyr::rename(pft_id = id) %>% filter(pft_id == pftid) %>%
  ...
  collect() %>% mutate(species_name = tolower(scientificname))

problem_species <- dat_test %>%
  filter(!species %in% sp$species) %>%
  filter(!species %in% PFT_species$species_name)

problem_species %>% pull() %>% unique
