...
However, the normal distribution fits the prior data the best and the 95% CI falls below 0. 
So for now I'm going to stick with the normal distribution. 
...
```
So in this case, the log normal distribution fits the prior data better (with a lower RMSE)
but the normal distribution fits the default values better. 
I'm not really sure which is best ... 
Down below I'll perform the next set of calculations using both the log normal and normal distributions. 
At the end of the day, I think the distribution that fits better with the prior data (ie log normal) produces better looking results.
Which is nice because I'd like to go by default and always choose the model with the better fit to the prior data. 
...
I am also going to calculate with both the normal and the log normal distribution for leaf density to see how it affects the final distribution. 
...
From this pot, we can see that just using the equations in the ED code makes 
...
in relation to the default value and the three values that Felicien gave me. 
...
which is the log normal distribution instead of trying to come up with a reason 
why we should make leaf density normal instead. 
...
Once again the 95% confidence interval is below 0 so I'm ok with using normal even though psi should be negative. 
